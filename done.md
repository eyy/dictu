# dictu — what has been done

every closed item, in the order it stood in `roadmap.md`. moved here so the task list is a
list of tasks again: at the split it was 1,577 lines, of which 1,344 were finished work.

numbers are stable and never reused, so an open item that argues with `#59` means the `#59`
below. nothing has been rewritten — these are the notes as they were written when the work
landed. the fuller version of any of them is the commit that closed it (`git log --grep`).
